...
st.title("GEN AI Course (Title)")
st.header("GEN AI Course (Header)")
st.subheader("GEN AI Course (Sub-Header)")
st.write("GEN AI Course (Text)")
st.markdown("GEN AI Course (Markdown)")
st.caption("GEN AI Course (Caption)")
...
st.sidebar.title("GEN AI Course ")
st.sidebar.image("xyz.jpeg")
...
df = pd.DataFrame(
    np.random.randn(10, 5), columns = (f"col {i}" for i in range(5))
)
st.table(df)
# df = pd.DataFrame(
...
# % formatting → purani style, ab zyada use nahi hoti
# .format() → thodi modern, backward compatible
# f"" → new, sabse clean aur readable
